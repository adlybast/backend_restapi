...
class ClassModel extends CI_Model {
	...
	{
	    return $this->db->select('id,name,capacity,created_at,update_at')->from('class')->order_by('id','asc')->get()->result();
	}

	public function class_detail_data($id)
	{
	    return $this->db->select('id,name,capacity,created_at,update_at')->from('class')->where('id',$id)->order_by('id','asc')->get()->row();
	}
